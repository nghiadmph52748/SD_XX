<template>
  ...
  <div class="add-form">
    ...
    <form @submit.prevent="fetchCreate">
      ...
      <div>
        ...
        <input type="radio" name="Trạng thái" :value="false" v-model="newLoaiMua.deleted" />Hoạt động
        <input type="radio" name="Trạng thái" :value="true" v-model="newLoaiMua.deleted" />Không hoạt động
      </div>
  ...
  <div class="edit-form" v-if="showEditForm">
    ...
    <form @submit.prevent="fetchUpdate">
      ...
      <div>
        <label for="">Trạng thái</label>
        <input type="radio" name="editTrạng thái" :value="false" v-model="selectedLoaiMua.deleted" />Hoạt động
        <input type="radio" name="editTrạng thái" :value="true" v-model="selectedLoaiMua.deleted" />Không hoạt động
      </div>
  ...
  <div v-if="totalPages > 1" class="pagination-wrapper">
    <div class="pagination-info">
      Hiển thị {{ startIndex + 1 }} - {{ endIndex }} của {{ totalItems }} loại mua
    </div>
    ...
    <div class="modal-content" @click.stop>
      ...
      <div class="modal-body">
        ...
        <div class="detail-row">
          ...
          <div class="detail-value">{{ selectedLoaiMua.moTa || 'Không có mô tả' }}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Trạng thái:</div>
          <div class="detail-value">
            <span :class="selectedLoaiMua.deleted ? 'status-inactive' : 'status-active'">
        ...
        <div class="detail-row">
          ...
          <div class="detail-value">{{ formatDate(selectedLoaiMua.createdAt) || 'Không có thông tin' }}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Ngày cập nhật:</div>
          <div class="detail-value">{{ formatDate(selectedLoaiMua.updatedAt) || 'Không có thông tin' }}</div>
        </div>
      </div>
...
const fetchCreate = async () => {
  ...
  try {
    ...
    errorMessage.value = "Lỗi khi thêm: " + (error.message || "Không thể tạo loại mua");
  } finally {
...
const fetchDelete = async (id) => {
  ...
  try {
    ...
    errorMessage.value = "Lỗi khi xóa: " + (error.message || "Không thể xóa loại mua");
    setTimeout(() => {
      errorMessage.value = null;
    }, 3000);
  }
