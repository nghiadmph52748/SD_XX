<template>
  ...
  <div class="management-header">
    <div class="title-section">
      ...
      <h3 class="sub-title">Quản lý chất liệu</h3>
    </div>
  ...
  <div v-if="showAddForm" class="modal-overlay" @click="closeAddForm">
    <div class="modal-content add-modal" @click.stop>
      <div class="modal-header add-header">
        <h3><i class="fas fa-plus"></i> Thêm Chất Liệu Mới</h3>
      ...
      <div class="modal-body">
        <form @submit.prevent="fetchCreate">
          <div class="detail-row">
            <div class="detail-label">Tên chất liệu:</div>
            <div class="detail-value">
              <input v-model="newChatLieu.tenChatLieu" type="text" required class="detail-input" />
            </div>
          </div>
  ...
  <div v-if="totalPages > 1" class="pagination-wrapper">
    <div class="pagination-info">
      Hiển thị {{ startIndex + 1 }} - {{ endIndex }} của {{ totalItems }} chất liệu
    </div>
  ...
  <div v-if="showDetailModal" class="modal-overlay" @click="closeDetailModal">
    <div class="modal-content" @click.stop>
      ...
      <div class="modal-body">
        ...
        <div>
          <div class="detail-row">
            ...
            <div class="detail-value">
              <input 
                v-model="selectedChatLieu.tenChatLieu" 
                type="text" 
                required 
                class="detail-input"
              />
          ...
          <div class="detail-row">
            ...
            <div class="detail-value">
              <div class="radio-group">
                <label class="radio-label">
                  <input type="radio" name="detailTrạng thái" :value="false" v-model="selectedChatLieu.deleted" />
                  <span>Hoạt động</span>
                </label>
                <label class="radio-label">
                  <input type="radio" name="detailTrạng thái" :value="true" v-model="selectedChatLieu.deleted" />
                  <span>Không hoạt động</span>
                </label>
              </div>
            </div>
      ...
      <div class="modal-footer">
        <button class="btn btn-success" @click="fetchUpdate" :disabled="uploading">
...
const filteredChatLieus = computed(() => {
  let filtered = [...ChatLieus.value];

  if (searchQuery.value.trim()) {
    const query = searchQuery.value.toLowerCase();
    filtered = filtered.filter(item => 
      item.tenChatLieu?.toLowerCase().includes(query)
    );
  }
...
const totalItems = computed(() => filteredChatLieus.value.length);
...
const fetchDelete = async (id) => {
  ...
  // Lấy tên chất liệu để hiển thị trong thông báo
  const item = ChatLieus.value.find(item => item.id === id);
  if (item) {
    deleteItemName.value = item.tenChatLieu || 'Chất liệu';
  }
};
...
const editFromDetail = () => {
  showDetailModal.value = false;
  openEditForm(selectedChatLieu.value);
};
