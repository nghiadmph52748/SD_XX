<template>
    ...
    <div class="add-form">
        ...
        <form @submit.prevent="fetchCreate">
            <div>
                ...
                <input v-model="newKichThuoc.tenKichThuoc" type="text" required />
            </div>
            <div>
                <label>Mô tả:</label>
                <input v-model="newKichThuoc.moTa" type="text" />
            </div>
            <div>
                <label for="">Trạng thái</label>
                <input type="radio" name="Trạng thái" :value="false" v-model="newKichThuoc.deleted" />Hoạt động
                <input type="radio" name="Trạng thái" :value="true" v-model="newKichThuoc.deleted" />Không hoạt động
            </div>
    ...
    <div class="edit-form" v-if="showEditForm">
        ...
        <form @submit.prevent="fetchUpdate">
            <div>
                <label>Tên kích thước:</label>
                <input v-model="selectedKichThuoc.tenKichThuoc" type="text" required />
            </div>
            <div>
                <label>Mô tả:</label>
                <input v-model="selectedKichThuoc.moTa" type="text" />
            </div>
            <div>
                <label for="">Trạng thái</label>
                <input type="radio" name="editTrạng thái" :value="false" v-model="selectedKichThuoc.deleted" />Hoạt động
                <input type="radio" name="editTrạng thái" :value="true" v-model="selectedKichThuoc.deleted" />Không hoạt
                động
            </div>
        ...
        <tbody>
            <tr v-for="(value, i) in paginatedKichThuocs" :key="value.id">
                ...
                <td>
                    ...
                    <button v-on:click="fetchDelete(value.id)" class="btn btn-delete btn-icon btn-sm"
                        :disabled="uploading" title="Xóa">
    ...
    <div v-if="totalPages > 1" class="pagination-wrapper">
        <div class="pagination-info">
            Hiển thị {{ startIndex + 1 }} - {{ endIndex }} của {{ totalItems }} kích thước
        </div>
        ...
        <div class="modal-content" @click.stop>
            ...
            <div class="modal-body">
                <div class="detail-row">
                    ...
                    <div class="detail-value">{{ selectedKichThuoc.id }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Tên kích thước:</div>
                    <div class="detail-value">{{ selectedKichThuoc.tenKichThuoc }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Mô tả:</div>
                    <div class="detail-value">{{ selectedKichThuoc.moTa || 'Không có mô tả' }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Trạng thái:</div>
                    <div class="detail-value">
                        <span :class="selectedKichThuoc.deleted ? 'status-inactive' : 'status-active'">
                            {{ selectedKichThuoc.deleted ? "Không hoạt động" : "Hoạt động" }}
                        </span>
                    </div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Ngày tạo:</div>
                    <div class="detail-value">{{ formatDate(selectedKichThuoc.createdAt) || 'Không có thông tin' }}
                    </div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Ngày cập nhật:</div>
                    <div class="detail-value">{{ formatDate(selectedKichThuoc.updatedAt) || 'Không có thông tin' }}
                    </div>
                </div>
            </div>
...
const paginatedKichThuocs = computed(() => {
    return KichThuocs.value.slice(startIndex.value, endIndex.value);
});
...
const fetchAll = async () => {
    try {
        ...
        totalItems.value = KichThuocs.value.length; // Update total items for pagination
    } catch (error) {
...
const fetchCreate = async () => {
    ...
    try {
        ...
        errorMessage.value = "Lỗi khi thêm: " + (error.message || "Không thể tạo kích thước");
    } finally {
...
const fetchUpdate = async () => {
    ...
    try {
        ...
        editErrorMessage.value = "Lỗi khi cập nhật: " + (error.message || "Không thể cập nhật kích thước");
    } finally {
        uploading.value = false;
    }
};
...
const fetchDelete = async (id) => {
    ...
    try {
        ...
        errorMessage.value = "Lỗi khi xóa: " + (error.message || "Không thể xóa kích thước");
        setTimeout(() => {
            errorMessage.value = null;
        }, 3000);
    }
