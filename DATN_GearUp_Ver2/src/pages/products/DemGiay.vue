<template>
  ...
  <div class="add-form">
    <h3>Thêm Đế Giày Mới</h3>
    <form @submit.prevent="fetchCreate">
      <div>
        <label>Tên đế giày:</label>
        <input v-model="newDemGiay.tenDemGiay" type="text" required />
      </div>
      <div>
        <label>Mô tả:</label>
        <input v-model="newDemGiay.moTa" type="text" />
      </div>
      <div>
        <label for="">Trạng thái</label>
        <input type="radio" name="Trạng thái" :value="false" v-model="newDemGiay.deleted" />Hoạt động
        <input type="radio" name="Trạng thái" :value="true" v-model="newDemGiay.deleted" />Không hoạt động
      </div>
  ...
  <div class="edit-form" v-if="showEditForm">
    ...
    <form @submit.prevent="fetchUpdate">
      <div>
        <label>Tên đế giày:</label>
        <input v-model="selectedDemGiay.tenDemGiay" type="text" required />
      </div>
      <div>
        <label>Mô tả:</label>
        <input v-model="selectedDemGiay.moTa" type="text" />
      </div>
      <div>
        <label for="">Trạng thái</label>
        <input type="radio" name="editTrạng thái" :value="false" v-model="selectedDemGiay.deleted" />Hoạt động
        <input type="radio" name="editTrạng thái" :value="true" v-model="selectedDemGiay.deleted" />Không hoạt động
      </div>
  ...
  <table class="table table-bordered">
    ...
    <tbody>
      <tr v-for="(value, i) in paginatedDemGiays" :key="i">
        <td>{{ i + 1 }}</td>
  ...
  <div v-if="totalPages > 1" class="pagination-wrapper">
    <div class="pagination-info">
      Hiển thị {{ startIndex + 1 }} - {{ endIndex }} của {{ totalItems }} đếm giày
    </div>
    ...
    <div class="modal-content" @click.stop>
      ...
      <div class="modal-body">
        <div class="detail-row">
          ...
          <div class="detail-value">{{ selectedDemGiay.id }}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Tên đế giày:</div>
          <div class="detail-value">{{ selectedDemGiay.tenDemGiay }}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Mô tả:</div>
          <div class="detail-value">{{ selectedDemGiay.moTa || 'Không có mô tả' }}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Trạng thái:</div>
          <div class="detail-value">
            <span :class="selectedDemGiay.deleted ? 'status-inactive' : 'status-active'">
              {{ selectedDemGiay.deleted ? "Không hoạt động" : "Hoạt động" }}
            </span>
          </div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Ngày tạo:</div>
          <div class="detail-value">{{ formatDate(selectedDemGiay.createdAt) || 'Không có thông tin' }}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Ngày cập nhật:</div>
          <div class="detail-value">{{ formatDate(selectedDemGiay.updatedAt) || 'Không có thông tin' }}</div>
        </div>
      </div>
...
const paginatedDemGiays = computed(() => {
  return DemGiays.value.slice(startIndex.value, endIndex.value);
});
...
const fetchAll = async () => {
  try {
    ...
    totalItems.value = DemGiays.value.length; // Update total items for pagination
  } catch (error) {
...
const editFromDetail = () => {
  showDetailModal.value = false;
  openEditForm(selectedDemGiay.value);
};
