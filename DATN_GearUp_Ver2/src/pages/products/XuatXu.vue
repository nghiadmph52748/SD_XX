<template>
  ...
  <div class="add-form">
    ...
    <form @submit.prevent="fetchCreate">
      <div>
        ...
        <input v-model="newXuatXu.tenXuatXu" type="text" required />
      </div>
      <div>
        <label>Mô tả:</label>
        <input v-model="newXuatXu.moTa" type="text" />
      </div>
      <div>
        <label for="">Trạng thái</label>
        <input type="radio" name="Trạng thái" :value="false" v-model="newXuatXu.deleted" />Hoạt động
        <input type="radio" name="Trạng thái" :value="true" v-model="newXuatXu.deleted" />Không hoạt động
      </div>
  ...
  <div class="edit-form" v-if="showEditForm">
    ...
    <form @submit.prevent="fetchUpdate">
      <div>
        <label>Tên xuất xứ:</label>
        <input v-model="selectedXuatXu.tenXuatXu" type="text" required />
      </div>
      <div>
        <label>Mô tả:</label>
        <input v-model="selectedXuatXu.moTa" type="text" />
      </div>
      <div>
        <label for="">Trạng thái</label>
        <input type="radio" name="editTrạng thái" :value="false" v-model="selectedXuatXu.deleted" />Hoạt động
        <input type="radio" name="editTrạng thái" :value="true" v-model="selectedXuatXu.deleted" />Không hoạt động
      </div>
    ...
    <div class="modal-content" @click.stop>
      ...
      <div class="modal-body">
        <div class="detail-row">
          ...
          <div class="detail-value">{{ selectedXuatXu.id }}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Tên xuất xứ:</div>
          <div class="detail-value">{{ selectedXuatXu.tenXuatXu }}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Mô tả:</div>
          <div class="detail-value">{{ selectedXuatXu.moTa || 'Không có mô tả' }}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Trạng thái:</div>
          <div class="detail-value">
            <span :class="selectedXuatXu.deleted ? 'status-inactive' : 'status-active'">
              {{ selectedXuatXu.deleted ? "Không hoạt động" : "Hoạt động" }}
            </span>
          </div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Ngày tạo:</div>
          <div class="detail-value">{{ formatDate(selectedXuatXu.createdAt) || 'Không có thông tin' }}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Ngày cập nhật:</div>
          <div class="detail-value">{{ formatDate(selectedXuatXu.updatedAt) || 'Không có thông tin' }}</div>
        </div>
      </div>
...
const fetchCreate = async () => {
  ...
  try {
    ...
    errorMessage.value = "Lỗi khi thêm: " + (error.message || "Không thể tạo xuất xứ");
  } finally {
...
const fetchUpdate = async () => {
  ...
  try {
    ...
    editErrorMessage.value = "Lỗi khi cập nhật: " + (error.message || "Không thể cập nhật xuất xứ");
  } finally {
    uploading.value = false;
  }
};
...
const fetchDelete = async (id) => {
  ...
  try {
    ...
    errorMessage.value = "Lỗi khi xóa: " + (error.message || "Không thể xóa xuất xứ");
    setTimeout(() => {
      errorMessage.value = null;
    }, 3000);
  }
