<template>
  ...
  <div v-if="totalPages > 1" class="pagination-wrapper">
    <div class="pagination-info">
      Hiển thị {{ startIndex + 1 }} - {{ endIndex }} của {{ totalItems }} chống nước
    </div>
...
const fetchCreate = async () => {
  if (!newChongNuoc.value.tenChongNuoc) {
    errorMessage.value = "Vui lòng nhập tên chống nước";
    return;
  }
...
const editFromDetail = () => {
  ...
  openEditForm(selectedChongNuoc.value); // Mở form edit
};
